var relearn_searchindex = [
  ...
  {
    "breadcrumb": "HUGO",
    "content": "Hugo 常用指令 brew install hugo - macOS install. hugo version hugo server - 启动本地服务器。 hugo --gc - 垃圾回收。 生成静态文件 hugo - 默认生成到 public/。 hugo -d docs - 生成静态文件，使用 docs/ 作为发布目录。 在配置文件 hugo.toml 中添加以下语句，可更改发布目录默认为 docs/（ 放到文件顶部）。\npublishDir = \"docs\" # 静态文件输出目录 Terminal mkdir tech_notes - 创建文件夹 cd tech_notes - 进入文件夹 touch docs/css/custom.css - 创建文件 rm -rf public/ - 删除文件夹 MkDocs mkdocs new [dir-name] - Create a new project. mkdocs serve - Start the live-reloading docs server. mkdocs build - Build the documentation site. mkdocs -h - Print help message and exit. Enable GitHub Pages for Hosting MkDocs After pushing your MkDocs project to GitHub, install the MkDocs GitHub Pages plugin:\npip install mkdocs-material mkdocs-git-revision-date-localized-plugin Add this to your mkdocs.yml file:\nplugins: - search - git-revision-date-localized Deploy your site:\nmkdocs gh-deploy",
    "description": "Hugo 常用指令 brew install hugo - macOS install. hugo version hugo server - 启动本地服务器。 hugo --gc - 垃圾回收。 生成静态文件 hugo - 默认生成到 public/。 hugo -d docs - 生成静态文件，使用 docs/ 作为发布目录。 在配置文件 hugo.toml 中添加以下语句，可更改发布目录默认为 docs/（ 放到文件顶部）。\npublishDir = \"docs\" # 静态文件输出目录 Terminal mkdir tech_notes - 创建文件夹 cd tech_notes - 进入文件夹 touch docs/css/custom.css - 创建文件 rm -rf public/ - 删除文件夹 MkDocs mkdocs new [dir-name] - Create a new project. mkdocs serve - Start the live-reloading docs server. mkdocs build - Build the documentation site. mkdocs -h - Print help message and exit. Enable GitHub Pages for Hosting MkDocs After pushing your MkDocs project to GitHub, install the MkDocs GitHub Pages plugin:",
    "tags": [],
    "title": "Quick lookup",
  ...
  {
    "breadcrumb": "HUGO",
    "content": "Share and collaborate your project Create a Repository Go to GitHub. Create a new repository for your project by clicking on the New in the Repositories. Name the repository and select whether it should be Public or Private. Check the box for Add a README file. Configure Git Account Check Currently Active Git Account View the account used by the current project (priority: project \u003e global).\ngit config --show-origin user.name git config --show-origin user.email View global configuration.\ngit config --global --list | grep user Configure Git Account Uniformly Global settings (default for all projects).\ngit config --global user.name \"Astra726\" git config --global user.email \"XXX\" Project-level settings (current project only).\ngit config user.name \"Astra726\" git config user.email \"XXX\" Push Your Project to GitHub On your local machine where the project exists:\n# Navigate to Your Project Directory cd /path/to/your/project # Delete the local .git folder rm -rf .git # Connect to a new remote repository git remote add origin https://github.com/Astra726/WJNote.git git remote -v # Check Git Status, to see which files have been modified or added git status # Initialize Git git init # Add your files to the Git staging area git add . # Commit your changes git commit -m \"Initial commit\" # Push the changes to GitHub git push -u origin main Collaborating Across Multiple Computers On any other computer, you (or collaborators) can clone the repository and continue editing:\n# Clone the repository: `git clone https://github.com/your-username/your-repository.git` # Pull changes on other machines before working: `git pull` 无法连接 Github 问题解决 同步仓库失败，报错提示如下：\nFailed to connect to github.com port 443 after 75016 ms: Couldn’t connect to server.\nHost github.com:443 was resolved.\n重置 Git 代理 git config --global --unset http.proxy git config --global --unset https.proxy 为终端配置代理 临时启用代理（替换为你的代理地址）\nexport http_proxy=\"http://127.0.0.1:7890\" export https_proxy=\"http://127.0.0.1:7890\" 测试是否生效\ncurl -v https://github.com 配置 GitHub Pages 使用 Hugo 输出 使用 docs/ 目录（推荐） 将 Hugo 生成的静态文件放入 docs/ 目录；\n在 GitHub 仓库设置中：进入 Settings → Pages。\n选择 Source: Deploy from a branch → Branch:main → Folder: /docs。\n点击 /Save。\nHugo 生成静态文件后不显示 CSS 检查 hugo.toml 文件中的 baseURL，应设为部署网址。 # 本地测试用 baseURL = \"http://localhost:1313/\" # Github Page baseURL = \"https://astra726.github.io/WJNote/\" 修改为基于根路径的绝对路径：查看 docs/ 下的 index.html 文件里的 css 引用路径，或者在页面使用 f12 进入开发者模式，console 下面查看哪些 css 文件引用出错。 错误示例：例如下面显示的 ./WJNote/ 为相对路径，而在部署到 GitHub Pages 后，应该使用的是从根路径开始的相对路径或绝对路径。\n解决办法：检查 hugo.toml 文件中的是否有设置 relativeURLs = true，删掉这句话。\n❌ 错误路径：\u003clink href=\"./WJNote/fonts/fontawesome/css/...\"\u003e\u003cnoscript\u003e ✅ 正确路径：\u003clink href=\"/WJNote/css/theme.min.css?1743775153\" rel=\"stylesheet\"\u003e",
  ...
  {
    "breadcrumb": "PYTHON",
    "content": "Mac 电脑装机，配置代码环境。\n设置使 Mac 中的 Python 默认指向 Python 3 检查当前 Python 版本和地址 打开终端，运行： python --version python3 --version which python which python3 创建符号链接，将 python 命令链接到 python3。 运行以下命令： # 删除现有的 Python 符号链接： sudo rm /usr/local/bin/python #创建新的符号链接，将 python 链接到 Python 3 的路径： sudo ln -s /Library/Frameworks/Python.framework/Versions/3.13/bin/python3 /usr/local/bin/python 重新运行：python --version，设置成功应该显示 Python 3.x。 安装 pip 检查 pip 是否已经安装：\npython -m ensurepip --upgrade 使用 get-pip.py 安装（如果尚未安装）：\n下载 get-pip.py 脚本： curl https://bootstrap.pypa.io/get-pip.py -o get-pip.py # 运行 get-pip.py 来安装 pip： python get-pip.py # 更新 pip： python -m pip install --upgrade pip vscode 配置解释器 按住 ctrl+shift+p （macOS: cmd+shift+p）输入文本查找并选择 Python:Select Inerpreter。 选择正确的 python 版本。 SSL 证书问题 pip 安装某些库时报错：There was a problem confirming the ssl certificate: …。\n解决办法：关掉代理。\n删除当前安装 python /bin/bash -c “$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/uninstall.sh)\"\n删除与 Python 相关的文件：\n删除全局 Python 可执行文件和链接： sudo rm -f /usr/local/bin/python3 sudo rm -f /usr/local/bin/python sudo rm -f /usr/local/bin/pip3 sudo rm -f /usr/local/bin/pip 删除残留的配置文件： 手动清理残留的配置文件（例如在 ~/.bash_profile, ~/.zshrc 等文件中设置的 Python 环境变量）。 Homebrew 安装 brew 打开终端，运行以下命令来安装 Homebrew（需要管理员密码）：\n/bin/bash -c \"$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh)\" # 验证 Homebrew 是否安装成功 brew --version 如果 curl 命令无法直接下载 Homebrew 安装脚本（Failed to connect to raw.githubusercontent.com port 443 after 1 ms: Couldn’t \u003e connect to server，），可以从浏览器直接下载该脚本手动运行：\n在浏览器中访问 URL：https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh。 将脚本下载到本地文件系统。 在终端中进入文件所在目录并运行：/bin/bash install.sh 手动添加 Homebrew 到 PATH 打开终端输入：\n# 编辑 ~/.zshrc 文件 nano ~/.zshrc #在文件的末尾添加以下内容（Homebrew 默认安装的路径 Apple Silicon Macs）。 export PATH=\"/opt/homebrew/bin:$PATH\" 保存并退出编辑器: Ctrl + X -\u003e Y -\u003e Enter。\n重新加载 ~/.zshrc 文件，使修改生效：source ~/.zshrc 。",
    "description": "Mac 电脑装机，配置代码环境。\n设置使 Mac 中的 Python 默认指向 Python 3 检查当前 Python 版本和地址 打开终端，运行： python --version python3 --version which python which python3 创建符号链接，将 python 命令链接到 python3。 运行以下命令： # 删除现有的 Python 符号链接： sudo rm /usr/local/bin/python #创建新的符号链接，将 python 链接到 Python 3 的路径： sudo ln -s /Library/Frameworks/Python.framework/Versions/3.13/bin/python3 /usr/local/bin/python 重新运行：python --version，设置成功应该显示 Python 3.x。 安装 pip 检查 pip 是否已经安装：\npython -m ensurepip --upgrade 使用 get-pip.py 安装（如果尚未安装）：\n下载 get-pip.py 脚本： curl https://bootstrap.pypa.io/get-pip.py -o get-pip.py # 运行 get-pip.py 来安装 pip： python get-pip.py # 更新 pip： python -m pip install --upgrade pip vscode 配置解释器 按住 ctrl+shift+p （macOS: cmd+shift+p）输入文本查找并选择 Python:Select Inerpreter。 选择正确的 python 版本。",
    "tags": [],
    "title": "Environment",
